...
panel_b_dat_early_fin1_all_ft <- panel_b_dat_early_fin_all_ft %>%
  ...
  dplyr::select(ftr_name_for_plots, ftr_name:omic_num)
...
meta_ftr_plot_early <- meta_ftr_early %>%
  ...
  tidylog::left_join(omics_names %>% 
                       dplyr::select(-omic_layer), by = "ftr_name") %>%
  dplyr::select(ftr_name_for_plots, ftr_name, omic, omic_num) %>%
...
cor_p_val_data_JIVE_with_zeros1_all <- cor_p_val_data_JIVE_with_zeros_all %>%
  ...
  dplyr::select(ftr_name_for_plots, everything()) %>%
  mutate(ftr_name_for_plots = rename_ftrs(ftr_name_for_plots) %>%
...
panel_b_dat_late_fin1_all <- panel_b_dat_late_fin_full %>%
  ...
  dplyr::select(ftr_name_for_plots, ftr_name:omic_num2) %>%
  mutate(ftr_name_for_plots = rename_ftrs(ftr_name_for_plots))
...
cormat1 <- cor(omics_df_reorder %>%dplyr::select(all_of(feature_name_reorder)))
