...
early <- result_hima_early_fin %>%
  ...
  dplyr::select(ftr_name, 'alpha*beta', pte_early, alpha, beta, sig_early) %>%
...
omics_df_subset <- omics_df %>%
  dplyr::select(unique(high_dim_res_fin$ftr_name)) 
...
correlation_matrix <- omics_df_subset %>% 
  dplyr::select(all_of(feature_name_reorder)) %>% 
  cor()
...
correlation_matrix_melt <- reshape2::melt(get_lower_tri(correlation_matrix), 
                                          na.rm = TRUE) %>%
  filter(Var1 != Var2) %>%
