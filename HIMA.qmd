...
source(fs::path(dir_proj, "functions", "hima_plot_functions.R"))
...
result_hima_early <- hima_early_integration(exposure = exposure,
                                            ...
                                            M.family = "gaussian")
# Plot Result
(hima_early_integration_plot(result_hima_early))
```
...
The full code for the `hima_intermediate_integration` and `hima_intermediate_integration_plot` function is provided in @sec-code_hima_int. *Note: For the actual analysis, we would normally set n_boot to a higher value (1000 or more). In the example code, it is set to 12 to improve the speed of the function.*
...
# plot
(hima_intermediate_integration_plot(result_hima_intermediate))
```
...
Combining all omics layers before analysis identifies the strongest mediating feature across all omics layers without accounting for the differences in underlying correlation structure. For this analysis, we used High Dimensional Mediation Analysis (HIMA), a penalization-based mediation method implemented in the R package HIMA [@RN33]. The full code for the `hima_late_integration` and `hima_late_integration_plot`function is provided in @sec-code_hima_late.
...
# plot
(hima_late_integration_plot(result_hima_late))
```
