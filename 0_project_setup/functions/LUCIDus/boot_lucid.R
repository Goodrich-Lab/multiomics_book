...
lucid_par_early <- function(data, indices, model, dimG, dimZ, dimCoY, dimCoG, prog) {
  ...
  if("try-error" %in% class(try_lucid)){
    ...
    if(is.null(names(try_lucid$res_Beta))) {
      Y_names <- paste0("cluster", 1:K)
    } else {
      Y_names <- names(try_lucid$res_Beta)
    }
...
# function to calculate parameters of LUCID in parallel model. use as statisitc input for
# boot function.
lucid_par_parallel <- function(data, Z_data, indices, model, dimG, dimZ, dimCoY, dimCoG, prog) {
  #display progress with each run of the function
  prog$tick()
  Sys.sleep(0.01)
