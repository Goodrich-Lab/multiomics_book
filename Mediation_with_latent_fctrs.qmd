...
In this analysis, `r n_80_pct` principal components explained >80% of the variance in the combined omics datasets. Of these `r n_80_pct` components, `r nrow(result_hima_pca_early_sig)` significantly mediated the relationship between maternal mercury and childhood liver injury (@fig-lf_early). 
...
top <- apply(ftr_cor_sig_pcs, 2, function(x) x %in% tail(sort(abs(x)), 10)) # Edit here
...
```{r JIVE, cache = TRUE, message = FALSE, warning = FALSE}
#| fig-width: 4
#| fig-height: 5
#| 
# conduct JIVE
...
cor_p_val_data_JIVE_with_zeros_all <- cor_p_val_data_JIVE_all %>%
  mutate(
    ...
    Correlation = if_else(in_ind_omic | str_detect(factor, "Joint"),
                          Correlation, 0)) %>%
  left_join(result_hima_jive_long %>% dplyr::select(component,components_ord, ind_joint_num),
...
top10 <- apply(ftr_cor_sig_pcs_jive_all, 2, function(x) x %in% tail(sort(abs(x)), 10))
selected_rows <- which(rowSums(top10) > 0)
ftr_cor_sig_pcs_top10 <- ftr_cor_sig_pcs_jive_all[selected_rows, ] %>%
  data.frame() %>% 
  rownames_to_column("feature")

panel_b_dat_jive_fin <- cor_p_val_data_JIVE_with_zeros1_all %>%
  filter(ftr_name %in% ftr_cor_sig_pcs_top10$feature)
...
This analysis identified 2 methylated CpG sites, 1 miRNA, 1 protein and 2 expressed gene transcript clutesrs (@fig-lf_late) 
...
pca_late_long <- result_hima_late_sig %>%
...
panel_b_dat_late_fin_full <- panel_b_dat_late_full %>%
  mutate(omic_num1 = get_omic_layer_numeric(feature),
...
top_late_int <- apply(ftr_cor_sig_pcs_late %>%
                        janitor::remove_empty(which = "rows"), 2,
...
# Add names of features for plotting
panel_b_dat_late_fin <- panel_b_dat_late_fin1_all %>% 
  filter(ftr_name %in% rownames(ftr_cor_sig_pcs_late))

# Plot
panel_b_late <- ggplot(data = panel_b_dat_late_fin,
                        aes(y = ftr_name,
                            x = pcs_ordered, 
                            fill = Correlation)) +
  geom_tile(color = "white") +
  facet_grid(omic_num1 ~ omic_num2, scales = "free", space = "free") +
  scale_fill_gradient2(low  = "blue",
                       mid  = "white",
                       high = "red",
                       midpoint = 0,
