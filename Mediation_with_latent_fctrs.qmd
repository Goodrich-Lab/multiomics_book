...
top <- apply(result_med_with_latent_fctrs_early$result_ftr_cor_sig_pcs_early %>% select(-omic_layer, -omic_num) %>% column_to_rownames("feature"), 2, function(x) x %in% tail(sort(abs(x)), 10)) 
...
# Select features for plots -------
# Select rows with values in the top 10 of their respective columns
top10 <- apply(result_med_with_latent_fctrs_JIVE$result_ftr_cor_sig_pcs_jive%>% select(-omic_layer, -omic_num) %>% column_to_rownames("feature"), 2, function(x) x %in% tail(sort(abs(x)), 10))
...
# Select features for plots -------
# Select rows with values in the top 10 of their respective columns
top_late_int <- apply(result_med_with_latent_fctrs_late$result_ftr_cor_sig_pcs_late %>% select(-feature,-omic_layer, -omic_num) %>% janitor::remove_empty(which = "rows"), 2,
